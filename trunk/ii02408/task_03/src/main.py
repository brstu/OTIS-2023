...
class Node:
    ...
    def change(self):
        ...
        button = Button(win, text="Edit", command=lambda: self.change_name(entry.get()))
...
def menu_add_vertex():
    ...
    add_button = Button(add_window, text = "Chose color",command = lambda: chose_color(color_lable))
...
def create_edge(entry_weight, entry_node1, entry_node2, window):
    try:
        ...
        messagebox.showerror("Error", "Edge weight should be number!")
    else:
...
def menu_add_edge():
    add_window = Tk()
    add_window.title("Adding edge")
    add_window.geometry("220x220+1050+250")
    add_window.wm_attributes('-topmost', 3)
    add_window.resizable(False, False)
    label = Label(add_window, text="Enter edge weight")
    entry_weight = Entry(add_window)
    label2 = Label(add_window, text="Enter name of first vertex")
    entry_node1 = Entry(add_window, text="Enter name of first vertex")
    label3 = Label(add_window, text="Enter name of second vertex")
    entry_node2 = Entry(add_window, text="Enter name of second vertex")
    add_button = Button(add_window, text = "Add color",command = lambda: chose_color(color_lable))
    color_button = Button(add_window, text="Add edge",command=lambda: create_edge(entry_weight, entry_node1, entry_node2, add_window))
    color_lable = Label(add_window,width=2,bg = "black")
    label.grid(row=0, column=0, sticky="ew")
...
graph = nx.Graph()  # Граф
root = Tk()
root.title("Graphs editor")
root.geometry("800x600+500+150")
root.resizable(False, False)
canvas = Canvas(root, width=795, height=596, bg="#00BFFF")
canvas.place(x=0, y=0)
#главное меню
main_menu = Menu()
file_menu = Menu()
file_menu1 = Menu()
main_menu.add_cascade(label="Working with vertices", menu=file_menu)
file_menu.add_command(label="Adding Vertices",command= menu_add_vertex)
file_menu.add_command(label="Adding an edge",command=menu_add_edge)
main_menu.add_cascade(label="Algorithms", menu=file_menu1)
file_menu1.add_command(label="Euler cycle",command=eulerian_cycle)
file_menu1.add_command(label="Shortest way",command=shortest_path)
root.config(menu=main_menu)
canvas.bind('<B1-Motion>', move_node)
canvas.bind('<Button-5>', change_name_or_weight)
canvas.bind('<Button-3>', change_color)
root.bind('<B4-Motion>', delete)
root.mainloop()