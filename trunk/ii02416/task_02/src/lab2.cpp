...
public:
   ...
   void lin(double y_c, double u, double t, double dt, size_t setting) // Change the type of 'setting' to 'size_t'
   {
      double a = 0.925;
      double b = 0.75;
      double k_p = 0.19;
      ...
      std::vector<double> a3(setting, static_cast<double>(t)); // Convert 't' to 'double'
      a1.push_back(y_c);
      ...
      for (double val : a2) // Range for-loop
      {
         std::cout << val << ",";
      }
      std::cout << "\n\nsetting:\n";
...
int main()
{
   pid T;
   std::cout << "setting:\n";
   size_t setting; // Change the type of 'setting' to 'size_t'
   std::cin >> setting;
   std::cout << "duration:\n";
   double duration;
   std::cin >> duration;
   ...
   double dis_time;
   std::cin >> dis_time;
   T.lin(0, 0, duration, dis_time, setting); // Pass 'setting' as a 'size_t' value
   return 0;
}