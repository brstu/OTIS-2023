...
//--------------------------------------------------------------------------
void __fastcall TForm2::OK1Click(TObject *Sender)
{
	int index1;
	edge buff;
	...
	int index2 = StrToInt(Form2->To_Edit1->Text);
	if (index2>vertices.size() || index2-1==index1)
		return;
	buff.X2 = vertices[index2-1].X;
	buff.Y2 = vertices[index2-1].Y;
	if(Form2->Weight_Edit2->Text=="")
		buff.weight = 0;
	else
		buff.weight = StrToInt(Form2->Weight_Edit2->Text);
	buff.Color = clBlack;
